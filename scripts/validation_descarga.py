...
# ==========================================
# CÓDIGO MODIFICADO: CONDADOS DE MINNESOTA
# ==========================================
...
temp_cache_path = os.path.join(out_path, 'temp_raw_osm')
utils.create_directory_if_not_exists(temp_cache_path)

for name in names:
    gdf_official = name2gdf[name]
    gdf_official = gdf_official.to_crs(epsg=4326)
    
    # Convertir CRS para gdf_official (siempre necesario para la intersección final)
    utm_crs = utils.get_utm_crs_from_geodataframe(gdf_official)
    gdf_official = gdf_official.to_crs(epsg=utm_crs)
    
    # ---------------------------------------------------------
    # SISTEMA DE CACHÉ LOCAL PARA EVITAR RE-DESCARGAR
    # ---------------------------------------------------------
    cache_file = os.path.join(temp_cache_path, f"{name}_raw_data.pickle")
    
    if os.path.exists(cache_file):
        print(f"\n---> [Caché encontrada] Cargando datos OSM previamente descargados para {name}...")
        with open(cache_file, 'rb') as handle:
            gdf_identified, footprint_id2features = pickle.load(handle)
    else:
        print(f"\n---> [Sin caché] Descargando y procesando geometría desde cero para {name}...")
        ...
        with open(cache_file, 'wb') as handle:
            pickle.dump((gdf_identified, footprint_id2features), handle, protocol=pickle.HIGHEST_PROTOCOL)
        print(f"---> Archivo guardado. La próxima vez cargará en segundos.")
    # ---------------------------------------------------------

    # =========================================================
    # PUNTO DE INSERCIÓN PARA TU FUTURO LLM
    # Aquí es donde 'footprint_id2features' tiene las etiquetas crudas 
    # y entra a la función de clasificación.
    # =========================================================

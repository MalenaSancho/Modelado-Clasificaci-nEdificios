...
def generate_gdf_with_segments(polygon, num_segments, tags):
    """
    Genera un GeoDataFrame descargando datos OSM por segmentos.

    MODIFICACIÓN:
    En el código original, si fallaba una descarga, el número de segmentos
    se reducía automáticamente. Eso puede empeorar los timeouts, porque
    menos segmentos implica consultas más grandes a Overpass.

    En esta versión:
    - mantenemos el número de segmentos indicado;
    - mostramos el error real;
    - no reducimos automáticamente el número de segmentos.
    """
    segments = _segment_polygon(polygon, num_segments)

    print(f"Descargando OSM en {len(segments)} segmentos...")

    gdf, footprint_id2features, gdf_not_used = generate_gdf_from_segments(
        segments,
        tags
    )
...
def generate_gdf_from_segments(segments, tags):
    ...
    for segment in tqdm(segments):
        ...
        for attempt in range(3):
            try:
                print(f"\nDescargando segmento. Intento {attempt + 1}/3")
                gdf_raw = ox.features_from_polygon(segment, tags)
                break

            except Exception as e:
                print(f"Error descargando segmento en intento {attempt + 1}/3:")
                print(e)
                ...
                if attempt < 2:
                    ...
                    print("No se pudo descargar este segmento tras 3 intentos.")
                    raise e
